...
/**
 * 基于MarkovMelodyGenerator，会随机的出现高音
 */
public class MarkovMelodyGeneratorVariation implements MelodyGenerator {
	...
	public void generateMelody(UniquePart uniquePart,
		...
		for (int i = 0; i < uniquePart.getEventsCount() - 1; i++) {
			...
			//按一定概率出高音
			if (test(pitchParameter/200.0-0.2)) {
				int newNote = note + Common.getRandomInteger(7, 14);
				newNote = uniquePart.alignPitchToHarmonic(i, newNote);
				uniquePart.setEventPitch(i, newNote);
			}
			else {
