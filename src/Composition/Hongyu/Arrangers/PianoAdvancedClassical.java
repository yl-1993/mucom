...
import Composition.Hongyu.Renderers.ChordalMelody;
...
public class PianoAdvancedClassical implements Arranger {
		...
		for (int i = 0; i < musicDescription.getPartsCount(); i++) {
			if (musicDescription.getPart(i).getArrangeHint() == 0)
			{
				...
				musicDescription.addRenderEvent(new ArpeggioChords(), 5,
						...
						musicDescription.getPartEndBar(i) - 1, 0, //+ off[0],
						new Time(0, 0), 0.5);
			...
			{
				//飘在高音区的旋律
				musicDescription.addRenderEvent(new ChordalMelody(), 0,
						musicDescription.getPartStartBar(i),
						musicDescription.getPartEndBar(i), 2,
						new Time(0, 0), 0.5);
				//普通的旋律
				...
				musicDescription.addRenderEvent(new ArpeggioChords(), 5,
						musicDescription.getPartStartBar(i),
						musicDescription.getPartEndBar(i) - 1, 0, // + off[3],
						new Time(0, 0), 0.4);

			}
			//每个Part的结尾部分的两个音
			musicDescription
					.addRenderEvent(new SimpleChords(), 4, 
							musicDescription.getPartEndBar(i) - 1, 
							musicDescription.getPartEndBar(i), 
							1, // + (off[musicDescription.getPart(i).getArrangeHint()] % 2), 
							new Time(0, 0), 0.6);
			...
			if (musicDescription.getPart(i).getArrangeHint() == 3) {
				...
				musicDescription.addRenderEvent(new ArpeggioChords(), 2,
						musicDescription.getPartStartBar(i), 
						musicDescription.getPartEndBar(i) - 1, 
						0, //off[musicDescription.getPart(i).getArrangeHint()], 
						new Time(0, 0), 0.5);
				//稳定的单重音-高
				musicDescription.addRenderEvent(new RandomBassExtended(), 3,
						musicDescription.getPartStartBar(i),
